  ...
  {J0:while (pt[self] < STOP)
      {either 
       Recv:{ \* local or receive event
            ...
            pt[self] := pt[self] +1;
            vc[self] := SetMax({pt[self],vc[self],msg[self][self]+1});      \* Update own time with max as specified
            m := 1;
             J1:while(m  <=  N){
                    if( m # self)
                    { vc[m] := SetMax({vc[m],msg[self][m], pt[self] - EPS} ) };  \* Update times of other entries accordingly....
                    m := m+1;
            };
         ...
         await(\A k \in Procs: pt[self] < pt[k]+ EPS); \* NTP clock sync
          ...
          if (vc[self]<pt[self]) vc[self]:=pt[self];
            ...
            with(k \in Procs \ {self})
                { msg[k]:=  vc  };     \* Pass message to be sent to any random process
        }    
      } 
  }
}
...
Recv(self) == /\ pc[self] = "Recv"
              ...
              /\ vc' = [vc EXCEPT ![self][self] = SetMax({pt'[self],vc[self][self],msg[self][self]+1})]
              /\ m' = [m EXCEPT ![self] = 1]
...
Boundedc == (\A k,p \in Procs: vc[k][k] >= vc[p][k]) 
\*Boundedness
